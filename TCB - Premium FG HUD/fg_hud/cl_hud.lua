...
plyMeta.drawPlayerInfo = function( self )
	...
	draw.DrawText( "Rank: " .. string.upper( string.sub( g, 1, 1 ) ) .. string.sub( g, 2 ), "FG_JaapokkiRegular_20", x, y-h+69, Color( 50, 50, 50, 255 ), 1 )
	...
	-----------------------------------------------------------]]
	if HUD.Modules["level"] then
		local level = LocalPlayer():getDarkRPVar('level') or 0
