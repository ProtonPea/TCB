...
function PANEL:DoClick()
	...
	if table.HasValue( GroupModerator, LocalPlayer():GetNWString( "usergroup" ) ) then
		...
		--> Spectate
		local spec = Option:AddOption( "Spectate", function() RunConsoleCommand( "_FAdmin", "Spectate", self.Player:UserID() ) end ):SetIcon( "icon16/eye.png" )

		Option:AddSpacer()

		--> Unarrest
		local unarrest = Option:AddOption( "Unarrest", function() RunConsoleCommand( "rp_unarrest", self.Player:Nick() ) end ):SetIcon( "icon16/link_break.png" )
		...
		banjob_pnl:SetIcon( "icon16/user_delete.png" )
			...
			for k,v in SortedPairsByMemberValue( RPExtraTeams, "name" ) do
				...
				jobban:AddOption( "2 minutes",     function() RunConsoleCommand( "rp_teamban", self.Player:UserID(), k, 120 )  end )
				jobban:AddOption( "Half an hour",  function() RunConsoleCommand( "rp_teamban", self.Player:UserID(), k, 1800 ) end )
				jobban:AddOption( "An hour",       function() RunConsoleCommand( "rp_teamban", self.Player:UserID(), k, 3600 ) end )
				jobban:AddOption ("Until restart", function() RunConsoleCommand( "rp_teamban", self.Player:UserID(), k, 0 )    end )
			end
		...
		unbanjob_pnl:SetIcon( "icon16/user_add.png" )
			...
			for k,v in SortedPairsByMemberValue( RPExtraTeams, "name" ) do
				unbanjob:AddOption( v.name, function() RunConsoleCommand( "rp_teamunban", self.Player:UserID(), k ) end )
			end
		...
		tele_pnl:SetIcon( "icon16/arrow_out.png" )
			...
			tele:AddSpacer()
			tele:AddOption( "Return", function() RunConsoleCommand( "ulx", "return", self.Player:Nick() ) end ):SetIcon( "icon16/arrow_redo.png" )
