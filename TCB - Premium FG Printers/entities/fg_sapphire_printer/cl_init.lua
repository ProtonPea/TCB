...
function ENT:Draw()
	...
	if money == 0 then moneycolor = SapphirePrinter.PrinterColor else moneycolor = Color( 0, 255, 0, 255 ) end
	...
	cam.Start3D2D( Pos + AngTop:Up() * 10.7, AngTop, 0.1 )
		...
		draw.DrawText( SapphirePrinter.PrinterName, "FG_JaapokkiRegular_50", -164+308/2+12, -135, Color( 255, 255, 255, 255 ), 1 )
	...
	cam.Start3D2D( Pos + AngBot:Up() * 16.3, AngBot, 0.1 )
		...
		draw.RoundedBox( 0, -75, -92+26*0+2, (140)/10*self:Getlvl_speed(), 20, Color( SapphirePrinter.PrinterColor.r, SapphirePrinter.PrinterColor.g, SapphirePrinter.PrinterColor.b, 150 ) )
		draw.RoundedBox( 0, -75, -92+26*1+2, (140)/10*self:Getlvl_quality(), 20, Color( SapphirePrinter.PrinterColor.r, SapphirePrinter.PrinterColor.g, SapphirePrinter.PrinterColor.b, 150 ) )
		draw.RoundedBox( 0, -75, -92+26*2+2, (140)/10*self:Getlvl_cooler(), 20, Color( SapphirePrinter.PrinterColor.r, SapphirePrinter.PrinterColor.g, SapphirePrinter.PrinterColor.b, 150 ) )
...
local function PrinterMenu()
	...
	Frame:SetTitle( SapphirePrinter.FrameTitle .. " - Printer Menu" )
	...
	if !printer or !IsValid( printer ) or printer == nil then Frame:Close() end

	local lvl_speed		= printer:Getlvl_speed()	or 0
	local lvl_quality	= printer:Getlvl_quality()	or 0
	local lvl_cooler	= printer:Getlvl_cooler()	or 0

	local data_power	= printer:Getdata_power()	or 0
	local data_money	= printer:Getdata_money()	or 0
	...
	Power.Paint = function( pnl, w, h )
		...
		--> Variables
		local varPower = printer:Getdata_power() or 0
	...
	startButton.DoClick = function()
		if printer:Getdata_power() == 0 then

			net.Start( "fg_printer_" .. SapphirePrinter.PrinterID .. "_power" )
	...
	stopButton.DoClick = function()
		if printer:Getdata_power() == 1 then
	...
	Money.Paint = function( pnl, w, h )
		...
		--> Variables
		local varMoney = printer:Getdata_money() or 0
	...
	collectButton.DoClick = function()
		if printer:Getdata_money() != 0 then

			net.Start( "fg_printer_" .. SapphirePrinter.PrinterID .. "_money" )
				net.WriteEntity( printer )
			net.SendToServer( LocalPlayer() )
	...
	--> Upgrade Price
	if SapphirePrinter.PrinterSpeed[ lvl_speed + 1 ] then
		local speedNextP = SapphirePrinter.DefaultQuality / 100 * SapphirePrinter.PrinterSpeed[ lvl_speed + 1 ][3]
	...
	for i = 1,10 do
		...
		star.Paint = function( pnl, w, h )
			...
			--> Count
			if SapphirePrinter.PrinterSpeed[ i ][4] == 1 then
				draw.RoundedBox( 4, 4, 4, w-8, h-8, Color( 155, 89, 182, 150 ) )
			elseif SapphirePrinter.PrinterSpeed[ i ][4] == 2 then
	...
	speedUpgrade.DoClick = function()
		...
		net.Start( "fg_printer_" .. SapphirePrinter.PrinterID .. "_upgrade_speed" )
			net.WriteEntity( printer )
		net.SendToServer( LocalPlayer() )
		...
		timer.Simple( 0.5, function()
			...
			for k,v in pairs( speedStars ) do
				if v[1] <= printer:Getlvl_speed() then
			...
			surface.SetFont( "FG_JaapokkiRegular_19" )
			speedLevel = "Level: " .. printer:Getlvl_speed()
			speedLW, speedLH = surface.GetTextSize( speedLevel )

			--> Upgrade Price
			if SapphirePrinter.PrinterSpeed[ printer:Getlvl_speed() + 1 ] then
				local speedNextP = SapphirePrinter.DefaultQuality / 100 * SapphirePrinter.PrinterSpeed[ printer:Getlvl_speed() + 1 ][3]
				speedPrice = "Price: $" .. speedNextP
			else
				speedPrice = "Max Level Reached"
			end
	...
	--> Upgrade Price
	if SapphirePrinter.PrinterQuality[ lvl_quality + 1 ] then
		local qualityNextP = SapphirePrinter.DefaultQuality / 100 * SapphirePrinter.PrinterQuality[ lvl_quality + 1 ][3]
	...
	for i = 1,10 do
		...
		star.Paint = function( pnl, w, h )
			...
			--> Count
			if SapphirePrinter.PrinterQuality[ i ][4] == 1 then
				draw.RoundedBox( 4, 4, 4, w-8, h-8, Color( 155, 89, 182, 150 ) )
			elseif SapphirePrinter.PrinterQuality[ i ][4] == 2 then
				draw.RoundedBox( 4, 4, 4, w-8, h-8, Color( 231, 76, 60, 150 ) )
			end
	...
	qualityUpgrade.DoClick = function()
		...
		net.Start( "fg_printer_" .. SapphirePrinter.PrinterID .. "_upgrade_quality" )
			net.WriteEntity( printer )
		net.SendToServer( LocalPlayer() )
		...
		timer.Simple( 0.5, function()
			...
			for k,v in pairs( qualityStars ) do
				if v[1] <= printer:Getlvl_quality() then
			...
			surface.SetFont( "FG_JaapokkiRegular_19" )
			qualityLevel = "Level: " .. printer:Getlvl_quality()
			qualityLW, qualityLH = surface.GetTextSize( qualityLevel )

			--> Upgrade Price
			if SapphirePrinter.PrinterQuality[ printer:Getlvl_quality() + 1 ] then
				local qualityNextP = SapphirePrinter.DefaultQuality / 100 * SapphirePrinter.PrinterQuality[ printer:Getlvl_quality() + 1 ][3]
				qualityPrice = "Price: $" .. qualityNextP
			else
				qualityPrice = "Max Level Reached"
			end
	...
	--> Upgrade Price
	if SapphirePrinter.PrinterCooler[ lvl_cooler + 1 ] then
		local coolerNextP = SapphirePrinter.DefaultQuality / 100 * SapphirePrinter.PrinterCooler[ lvl_cooler + 1 ][3]
	...
	for i = 1,10 do
		...
		star.Paint = function( pnl, w, h )
			...
			--> Count
			if SapphirePrinter.PrinterCooler[ i ][4] == 1 then
				draw.RoundedBox( 4, 4, 4, w-8, h-8, Color( 155, 89, 182, 150 ) )
			elseif SapphirePrinter.PrinterCooler[ i ][4] == 2 then
				draw.RoundedBox( 4, 4, 4, w-8, h-8, Color( 231, 76, 60, 150 ) )
			end
	...
	coolerUpgrade.DoClick = function()
		...
		net.Start( "fg_printer_" .. SapphirePrinter.PrinterID .. "_upgrade_cooler" )
			net.WriteEntity( printer )
		net.SendToServer( LocalPlayer() )
		...
		timer.Simple( 0.5, function()
			...
			for k,v in pairs( coolerStars ) do
				if v[1] <= printer:Getlvl_cooler() then
			...
			surface.SetFont( "FG_JaapokkiRegular_19" )
			coolerLevel = "Level: " .. printer:Getlvl_cooler()
			coolerLW, coolerLH = surface.GetTextSize( coolerLevel )

			--> Upgrade Price
			if SapphirePrinter.PrinterCooler[ printer:Getlvl_cooler() + 1 ] then
				local coolerNextP = SapphirePrinter.DefaultQuality / 100 * SapphirePrinter.PrinterCooler[ printer:Getlvl_cooler() + 1 ][3]
				coolerPrice = "Price: $" .. coolerNextP
			else
				coolerPrice = "Max Level Reached"
			end
...
net.Receive( "fg_printer_" .. SapphirePrinter.PrinterID .. "_menu", PrinterMenu )